    ...
    report = {
        ...
        "strict_control": {
            ...
            "tied_parameter_observation_scope": {
                "optimizer_excludes": ["transformer.wte.weight", "lm_head.weight"],
                "optimizer_includes": "every other unique parameter returned by model.parameters()",
                "optimizer_state_for_tied_parameter": "excluded",
                "full_model_parameters_include": ["model.transformer.wte.weight", "model.lm_head.weight"],
                "full_gradient_includes": ["gradient.transformer.wte.weight", "gradient.lm_head.weight"],
                "production_project_specific_mapping_rule_added": False,
                "reason": "the two names identify one shared Parameter and strict canonicalization ABSTAINS when that object appears in an optimizer",
            },
        },
        "test_summary": test_summary,
        "hosted_ci": ci,
        "preservation": {
            "accepted_gate_0_through_4b_hashes_verified_unchanged": True,
            "verification_source": "scripts/verify_gate5.py checked every recorded SHA-256 before Gate 5 acceptance",
